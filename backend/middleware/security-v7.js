/**
 * Advanced Security Middleware - v7.0 CORRECTED
 * 🔥 FIXES: CSRF logic corrected, rate limit handler fixed, account lock with Redis
 */
...
/**
 * ✅ FIXED: CSRF Token Middleware
 * NOW CORRECTLY compares to PRE-STORED token, not new token
 */
export const csrfProtection = (req, res, next) => {
    ...
    // ✅ FIXED: Compare to PRE-STORED token, not new token
...
export const ipRateLimiter = rateLimit({
    ...
    // ✅ FIXED: Return response instead of throwing
    handler: (req, res) => {
...
try {
    ...
    logger.warn('Redis not available, using memory fallback', { error: err.message });
}
