/**
 * Input Sanitization Utility - v8.0 FINAL
 * 🔥 FIXES: Schema validation, password handling, unknown fields
 */
 ...
 * NOW THROWS ERROR for unknown fields if schema is provided
 * Prevents passing fields without schema validation
 */
export const sanitizeObject = (obj, schema = {}) => {
    ...
    // ✅ FIXED: If schema is provided, validate against it strictly
    // Don't allow fields without schema definition
    const hasSchema = Object.keys(schema).length > 0;
    
    for (const [key, value] of Object.entries(obj)) {
        // ✅ FIXED: Throw error for unknown fields if schema is provided
        ...
        try {
            if (!fieldSchema) {
                // No schema defined and no strict schema provided
                // Sanitize as string
...
/**
 * ✅ FIXED: Mask sensitive data for logging
...
export const maskSensitiveData = (data) => {
    ...
    if (masked.phone && typeof masked.phone === 'string') {
        masked.phone = `***${masked.phone.substring(masked.phone.length - 4)}`;
    }
